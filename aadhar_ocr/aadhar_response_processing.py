...
#     sentence_bbox = {}
#     adhaar_number = None
#     aadhaar_pattern = re.compile(r'\b\d{4}\s\d{4}\s\d{4}\s?\d{0,4}\b')
#     for line in lines_data:
#         match = aadhaar_pattern.search(line['text'])
#         if match:
#             adhaar_number = match.group().replace(" ", "")
#             # Store bounding box and confidence for Aadhaar number
#             sentence_bbox['Aadhaar_Number'] = (line['boundingBox'], str(round(line['confidence']*100))+"%")
#             break
...
#         adhaar_pattern = r'\d{4}\s*\d{4}\s*\d{4}'
...
#         adhaar_match = re.search(adhaar_pattern, lines['text'])
#         if adhaar_match and not adhaar_number:
#             adhaar_number = re.sub(r'\s+', ' ', adhaar_match.group(0))
#             # You might want to add it to bbox as well
#             avg_confidence = address_line_bbox_check(lines)
#             if avg_confidence > 0.90:
#                 sentence_bbox["Aadhaar_Number"] = (lines['boundingBox'], str(round(avg_confidence*100))+"%")
#         #print("sentence bbox: ",sentence_bbox)
...
#             final_address_confidence = statistics.mean(address_confidence)*100
#             return {'Address': final_address,'Aadhaar_Number': adhaar_number,  "Bounding_Box":sentence_bbox,"Accuracy":final_address_confidence}
#         #print("final_address_confidence: ",final_address_confidence)
#         else:
#             pass    
#         #return {'Address': final_address, "Accuracy":final_address_confidence}
...
# #     if front_or_back_address_check:
# #         # Address is in the front format
# #         address, address_confidence, final_address_confidence, sentence_bbox = adhaar_front_get_address(lines_data)
        
# #         # Extract Aadhaar number from the text
# #         aadhaar_number, aadhaar_bbox, aadhaar_conf = extract_aadhaar_number_from_lines(lines_data)
        
# #         # Add Aadhaar number bounding box if found
# #         if aadhaar_number and aadhaar_bbox and aadhaar_conf:
# #             sentence_bbox['Aadhaar_Number'] = (aadhaar_bbox, str(round(aadhaar_conf*100))+"%")
        
# #         return {
# #             'Address': address,
# #             'Adhaar_Number': aadhaar_number,
# #             'Bounding_Box': sentence_bbox,
# #             'Accuracy': final_address_confidence
# #         }
    
# #     if not front_or_back_address_check:
# #         # Address is in back format
# #         width = data['analyzeResult']['readResults'][0]['width']
# #         xtl = ytl = xtr = ytr = xbr = ybr = xbl = ybl = 0
# #         global found_uidai
# #         found_uidai = False
# #         found_address = False
# #         found_pincode = False
...
# #             if fuzz.ratio('UNIQUE'.lower(), lines['words'][0]['text'].lower()) > 95:
# #                 uidai_index = lines_idex
# #                 threshold = abs(lines['boundingBox'][0] - (width / 3))
# #                 xtl, ytl = lines['boundingBox'][0] + threshold, lines['boundingBox'][1]
# #                 xtr, ytr = lines['boundingBox'][2], lines['boundingBox'][3]
# #                 xbr, ybr = lines['boundingBox'][4], lines['boundingBox'][4]
# #                 xbl, ybl = lines['boundingBox'][6] + threshold, lines['boundingBox'][7]
...
# #                             condition = words['boundingBox'][0] <= xtr and words['confidence'] >= 0.60
# #                         else:
# #                             condition = words['boundingBox'][0] >= xtl
...
# #                         if fuzz.ratio('ADDRESS'.lower(), address_text) >= 80 or found_address:
# #                             if address_with_word:
# #                                 try:
# #                                     text = words['text'].split(":")[1]
# #                                     address_with_word = False
...
# #         # Extract Aadhaar number from all lines
# #         aadhaar_number, aadhaar_bbox, aadhaar_conf = extract_aadhaar_number_from_lines(lines_data)
...
# #         print({'Address': final_address, 'Adhaar_Number': aadhaar_number, 'Bounding_Box': sentence_bbox, 'Accuracy': final_address_confidence})
# #         return {
# #             'Address': final_address,
# #             'Adhaar_Number': aadhaar_number,
# #             'Bounding_Box': sentence_bbox,
...
# # # Helper function to extract Aadhaar number from OCR lines
# # def extract_aadhaar_number_from_lines(lines_data):
# #     """
# #     Extract 12-digit Aadhaar number from OCR lines data.
# #     Supports formats: XXXX XXXX XXXX, XXXXXXXXXXXX, XXXX-XXXX-XXXX
# #     Returns tuple: (aadhaar_number, bounding_box, confidence)
# #     """
# #     aadhaar_patterns = [
...
# #     # Try to find Aadhaar number with its bounding box
# #     for line in lines_data:
# #         line_text = line['text']
...
# #             if match:
# #                 # Remove spaces and hyphens to get clean number
# #                 aadhaar_number = re.sub(r'[\s-]', '', match.group())
# #                 if len(aadhaar_number) == 12 and aadhaar_number[0] not in ['0', '1']:
# #                     # Calculate average confidence for this line
# #                     avg_confidence = address_line_bbox_check(line)
# #                     return aadhaar_number, line['boundingBox'], avg_confidence
    
# #     # If no pattern matched in individual lines, combine all text
# #     full_text = ' '.join([line['text'] for line in lines_data])
...
# #         if matches:
# #             # Remove spaces and hyphens to get clean number
# #             aadhaar_number = re.sub(r'[\s-]', '', matches[0])
# #             if len(aadhaar_number) == 12 and aadhaar_number[0] not in ['0', '1']:
# #                 return aadhaar_number, None, None
    
# #     # If no pattern matched, look for any 12 consecutive digits
# #     digits_only = re.sub(r'\D', '', full_text)
# #     if len(digits_only) >= 12:
# #         # Find the first occurrence of 12 consecutive digits
# #         for i in range(len(digits_only) - 11):
# #             potential_number = digits_only[i:i+12]
# #             # Basic validation: Aadhaar numbers don't start with 0 or 1
# #             if potential_number[0] not in ['0', '1']:
# #                 return potential_number, None, None
